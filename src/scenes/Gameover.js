...
export default class Gameover extends Phaser.Scene {
  ...
  preload() {
    this.load.audio('NumKey', './assets/sounds/NumKey.mp3');
  ...
  /* cargar SONIDOS */
  this.audioManager.add('NumKey', { volume: 0.5, loop: false });
...
createButton(x, y, key, text, sceneKey) {
  let button = this.add.image(x, y, key).setOrigin(0, 0).setInteractive();
  ...
  button.on('pointerdown', () => {
    /* crear delay para parar la escena y empezar  */
  ...
  /* variable para el color del tinte */
  const tintColor = 0xC5C5C5;

  /* hover button */
  button.on('pointerover', () => {
    this.audioManager.play('NumKey');
    button.setTint(tintColor);
    buttonText.setTint(tintColor);
  })
